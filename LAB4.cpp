// Verify Restoring Division Algorithm of computer system by using c or c++ codes.
#include <iostream>
#include <cmath>
using namespace std;
...
// Function to restore remainder if subtraction results in negative
...
void restoringDivision()
{
    ...
    for (int i = 0; i < SIZE; i++)
    {
        ...
        addBinary(rem, divisorComp); // Subtract divisor (add two’s complement)
        cout << "Subtract B: ";
        for (int j = 0; j < SIZE; j++)
            cout << rem[j];
        cout << endl;

        if (rem[0] == 1) // If remainder is negative, restore
        {
            restoreRemainder();
            quotient[SIZE - 1] = 0;
            cout << "Restore Remainder: ";
        }
        else
        {
            quotient[SIZE - 1] = 1;
...
int main()
{
    cout << "\t***** Restoring Division Algorithm *****\n";
